  ...
  return (
    <div
      className={`bg-white dark:bg-[#1d2226] ${
        modalPost ? "rounded-r-lg" : "rounded-lg border"
      } space-y-2 pt-2.5  border-gray-300 dark:border-none overflow-hidden`}
    >
      <div className="flex items-center px-2.5 cursor-pointer">
        <Avatar src={post.userImg} className="cursor-pointer !h-10 !w-10" />
      ...
      {post.input && (
        <div className="px-2.5 break-all md:break-normal">
          {modalPost || showInput ? (
            <p className="" onClick={() => setShowInput(false)}>
              {post.input}
            </p>
          ) : (
      ...
      <div className="flex justify-evenly items-start dark:border-t border-gray-600/80 text-black/60 dark:text-white/75">
        {modalPost ? (
